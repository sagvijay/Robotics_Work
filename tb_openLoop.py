...
class OpenLoopController(Node):
    ...
    def timer_callback(self):
        spent_time = time.time() - self.start_time #less than 10 seconds as mentioned above
        if spent_time < self.duration:
...
class OpenLoopController(Node):
    ...
    def timer_callback(self):
        ...
        if spent_time < self.acceleration_time:#less than acceleration time
            msg.linear.x = float(self.acceleration * spent_time) #vx= a*t
        elif spent_time < self.acceleration_time + self.constant_velocity_time: #b/w acceleration time and constant velocity time
            msg.linear.x = float(self.velocity)#constant velocity
        elif spent_time < self.total_time: # if time is in final cycle of deceleration
            deceleration_time = spent_time - self.acceleration_time - self.constant_velocity_time
